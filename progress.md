...
- Маршрутизация: входящее → клиника по instance_name (приоритет) → whatsapp_number
  (запасной); неактивная/ненайденная клиника → лог + тихий выход; ветка менеджера
  скоупится на найденную клинику; парсинг «завтра/сегодня» — в таймзоне клиники ✓
- Изоляция данных между клиниками покрыта тестами (messaging/test_routing.py) ✓
- Per-clinic webhook-токены и расширенный seed второй клиники — СЛЕДУЮЩИЕ промпты.
